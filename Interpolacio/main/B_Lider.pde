void Lider() {
  ...
  // Inicializar los puntos del líder
  ...
  // BOLA
  P0 = new PVector(lider[0].x, lider[0].y, lider[0].z);
...
void interpolate(float u, corba c) {
  ...
  posX = c.coefs[0].x + c.coefs[1].x * u + c.coefs[2].x * u2 + c.coefs[3].x * u3;
  posY = c.coefs[0].y + c.coefs[1].y * u + c.coefs[2].y * u2 + c.coefs[3].y * u3;
